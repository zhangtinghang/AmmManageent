...
<div class="app-container calendar-list-container">
    ...
    <el-table-column align="center" prop="roles" label="标题">
        <template slot-scope="scope">
    ...
    <el-table-column align="center" label="操作">
        <template slot-scope="scope">
          <el-row>
            ...
            <el-button size="mini" type="danger" @click="delNotice(scope.row._id)">删除</el-button>
        </el-row>
        </template>
    </el-table-column>
  </el-table>
...
import { changeStatus, delNotice, modNotice, getList } from '@/api/notice'
  ...
  export default {
    ...
    methods: {
      ...
      confirmEdit(row) {
          ...
          console.log(row)
          modNotice(user, token, row).then(response => {
      ...
      delNotice(_id){
        let token = store.getters.token;
        let user = store.getters.user;
        delNotice(user, token, _id).then(response => {
          if(response.errno == 0 && response.success==true){
              for (const v of this.tableData) {
                if (v._id === _id) {
                  const index = this.tableData.indexOf(v)
